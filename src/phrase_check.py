...
def extract1grams(listofwords):    
    gram_1=ngrams(listofwords,2)
    return gram_1
def extract2grams(listofwords):    
    gram_2=ngrams(listofwords,3)
    return gram_2
def extract3grams(listofwords):    
    gram_3=ngrams(listofwords,4)
    return gram_3
def extract4grams(listofwords):    
...
for sentence in trainsentenceswithoutpnctuation:
    ...
    if len(sentence) >= 2:
        g1=extract1grams(sentence)
    if len(sentence) >= 3:
        g2=extract2grams(sentence)
    if len(sentence) >= 4:
        g3=extract3grams(sentence)
    if len(sentence) >= 5:
        g4=extract4grams(sentence)
####find error words based on ngrams
    ...
    if len(Candidatewordstocheckfor) == 0:
        ...
        for i in cor:
            ...
            if len(sentence1) >= 2:
                g1=extract1grams(sentence1)
            if len(sentence1) >= 3:
                g2=extract2grams(sentence1)
            if len(sentence1) >= 4:
                g3=extract3grams(sentence1)
            if len(sentence1) >= 5:
                g4=extract4grams(sentence1)
            ##Compute MLE
            Overallsum=[]
            if len(sentence1) >= 2:
                for j in g1:
        ...
        with open(sys.argv[1], 'a') as out:
            out.write(sorted_x[0][0]+'\t')
            for sort in new_sorted[:3]:
                out.write(sort[0]+'\t'+str(sort[1])+'\t')
            out.write('\n')
    else:
        for cand in Candidatewordstocheckfor:
            print(cand)
            b,sc=spell_checker(cand)
            prioritydic={}
            ###Maximizing Likelihood function
            for c in b:
                s=sc[b.index(c)]
                ...
                print(c)
                word=c.lower()
                sentence1[sentence1.index(cand)]=word
                if len(sentence1) >= 2:
                    g1=extract1grams(sentence1)
                if len(sentence1) >= 3:
                    g2=extract2grams(sentence1)
